...
Zunanje* porazdeli(Notranje* zacetek, int k) {
    ...
    Notranje **seznam = calloc(1000, sizeof(Notranje*));
    int dolzinaSeznama = 0;
    
    Notranje *trenutno = zacetek;
        while(trenutno != NULL) {
        seznam[dolzinaSeznama] = trenutno;
        trenutno = trenutno->desno;
        seznam[dolzinaSeznama]->desno = NULL;
        dolzinaSeznama++;
    }

    Zunanje **zunanje = calloc(k, sizeof(Zunanje*));
    for (size_t i = 0; i < k; i++)
    {
        zunanje[i] = calloc(1, sizeof(Zunanje));
        if(i > 0) zunanje[i-1]->dol = zunanje[i];
    }

    for (size_t i = 0; i < dolzinaSeznama; i++)
    {
        int index = i % k;
        
        if (zunanje[index]->prvo == NULL)
        {
            zunanje[index]->prvo = seznam[i];
            zunanje[index]->zadnje = seznam[i];
        }
        else
        {
            zunanje[index]->zadnje->desno = seznam[i];
            zunanje[index]->zadnje = seznam[i];
