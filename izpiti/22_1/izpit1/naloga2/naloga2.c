...
Vozlisce** tabelaSeznamov(int* t, int d, int n) {
    Vozlisce** tabelaKazalcev = calloc(n, sizeof(Vozlisce*));
    int c;
    for (int i = 0; i < n; i++)
    {
        tabelaKazalcev[i] = calloc(d, sizeof(Vozlisce*));
        Vozlisce* node = tabelaKazalcev[i];
        c = 0;
        while (node->naslednje != NULL)
        {
            node->vTabelo = (t + c);
            c++;
            node = node->naslednje;
    ...
    return tabelaKazalcev;   // popravite
}
